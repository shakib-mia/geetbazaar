...
const DemoPDF = ({ data, setDemoVisible }) => {
  ...
  const handleConvertToPdf = async () => {
    ...
    pdf.save(
      `Revenue_Details_${profileData.first_name}_${profileData.last_name}`
    );
  ...
  return (
    <div className="h-screen w-screen backdrop-blur-md absolute top-0 left-0 z-[9999999] flex flex-col justify-center items-center gap-1">
      <div
        className="w-10/12 h-5/6 bg-grey-light p-2 rounded relative"
        ref={containerRef}
      >
        <button className="absolute -top-2 -right-2 text-black">&times;</button>
